...
app.put("/jokes/:id" , (req,res) => {
  ...
  // we can use id also as id == idex-1 
  const index = jokes.findIndex((joke) => (joke.id === id));

  jokes[index] = updateJokes;
...
app.patch("/jokes/:id" , (req,res) => {
  const id  = parseInt(req.params.id);
  const oldjokes = jokes.find((joke) => (joke.id === id));
  const updateJokes = {
    id : id,
    jokeText : req.body.text || oldjokes.text,
    jokeType : req.body.type || oldjokes,type
  }
  // we can use id also as id == idex-1 
  const index = jokes.findIndex((joke) => (joke.id === id));
  jokes[index] = updateJokes;
...
app.delete("/jokes/:id" , (req,res)=>{
  ...
  if(idx > -1){
    ...
    res
      .status(404)
      .json({error : `joke with ${id} 
        not found no jokes were deleted` });
  }
})
